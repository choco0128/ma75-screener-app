...
MOMENTUM_PRESETS = {
    "厳しめ": {
        "min_score_candidate": 80,
        "min_score_watch": 65,
        "min_ret_5d": 10.0,
        "min_ret_20d": 12.0,
        "high20_distance_max": 1.5,
        "volume_ratio_min": 1.8,
        "close_position_min": 0.65,
        "ma25_deviation_max": 20.0,
        "stop_loss_max_pct": 8.0,
        "upper_wick_max_pct": 35.0,
    },
    "普通": {
        "min_score_candidate": 70,
        "min_score_watch": 55,
        "min_ret_5d": 8.0,
        "min_ret_20d": 10.0,
        "high20_distance_max": 3.0,
        "volume_ratio_min": 1.5,
        "close_position_min": 0.60,
        "ma25_deviation_max": 25.0,
        "stop_loss_max_pct": 10.0,
        "upper_wick_max_pct": 45.0,
    },
    "ゆるめ": {
        "min_score_candidate": 60,
        "min_score_watch": 45,
        "min_ret_5d": 5.0,
        "min_ret_20d": 6.0,
        "high20_distance_max": 5.0,
        "volume_ratio_min": 1.2,
        "close_position_min": 0.50,
        "ma25_deviation_max": 35.0,
        "stop_loss_max_pct": 12.0,
        "upper_wick_max_pct": 55.0,
    },
}
...
def analyze_momentum(price_df, code, name, market, sector33, sector17, theme, params, use_ma200_filter):
    ...
    if score >= params["min_score_candidate"] and len(danger_signs) <= 1 and pd.notna(loss_pct) and loss_pct <= params["stop_loss_max_pct"]:
        buy_judge = "候補"
        priority = "A"
    elif score >= params["min_score_watch"] and len(danger_signs) <= 3:
        buy_judge = "監視"
        priority = "B"
    else:
        buy_judge = "見送り"
...
st.title("25日線初押し・75日線押し目・三角保ち合い・モメンタム スクリーニングアプリ")

st.write("出来高急増後の25日線初押し・75日線押し目・三角保ち合いブレイク初動・モメンタム初動/強勢継続をタブで切り替えて探せます。")
...
tab25, tab75, tab_triangle, tab_momentum = st.tabs(["25日線初押し", "75日線押し目", "三角保ち合いブレイク", "モメンタム"])
