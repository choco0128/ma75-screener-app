...
def run_screener(
    ...
    if len(results) == 0:
        ...
        sort_cols = ["_優先度順", "スコア", f"{ma_period}日線傾き%", f"{ma_period}日線距離%"]
        ...
        priority_filter = st.selectbox(
            "表示する優先度",
            ["すべて", "Aのみ", "A・Bのみ", "Bのみ", "Cのみ"],
            key=f"priority_filter_{ma_period}_{market_choice}"
        )

        display_df = result_df.copy()

        if priority_filter == "Aのみ":
            display_df = display_df[display_df["優先度"] == "A"]
        elif priority_filter == "A・Bのみ":
            display_df = display_df[display_df["優先度"].isin(["A", "B"])]
        elif priority_filter == "Bのみ":
            display_df = display_df[display_df["優先度"] == "B"]
        elif priority_filter == "Cのみ":
            display_df = display_df[display_df["優先度"] == "C"]

        display_df = display_df.drop(columns=["_優先度順"], errors="ignore")
        ...
        st.download_button(
            label="CSVダウンロード",
            data=csv,
            file_name=f"ma{ma_period}_pullback_{market_for_filename}_priority.csv",
            mime="text/csv",
...
with st.expander("このアプリの見方"):
    st.write(
        ...
        反発確認：
        ・あり：陽線、前日比上昇、前日高値超え、出来高回復などが複数揃っている
        ・弱め：一部だけ確認
        ・待ち：まだ反発が弱い
