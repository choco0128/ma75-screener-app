...
# ============================================================
# VCP（Volatility Contraction Pattern）用プリセット
# ============================================================
# 日足：50日EMAを中心に、Minervini系のStage 2 / VCP条件を数値化。
# 週足：50日EMAに近い10週EMAを使い、中期のVCPを探す。
VCP_DAILY_PRESETS = {
    "厳しめ": {
        "base_scan_bars": 100,
        "ema_period": 50,
        "ema_slope_bars": 20,
        "long_ma_period": 200,
        "require_long_ma": True,
        "pretrend_bars": 70,
        "min_prior_rise_pct": 30.0,
        "zigzag_threshold_pct": 4.0,
        "required_contractions": 3,
        "min_t1_pct": 10.0,
        "min_t2_pct": 6.0,
        "max_final_contraction_pct": 8.0,
        "contraction_ratio_max": 0.78,
        "min_final_low_age": 3,
        "max_final_low_age": 15,
        "tight_bars": 4,
        "tight_range_max_pct": 5.0,
        "volume_dryup_ratio_max": 0.75,
        "pivot_near_base_high_pct": 8.0,
        "monitor_distance_pct": 2.5,
        "breakout_margin_pct": 0.15,
        "breakout_volume_min": 1.7,
        "close_position_min": 0.60,
        "rs_lookback": 60,
        "rs_min_excess_pct": 5.0,
    ...
    "普通": {
        ...
        "ema_slope_bars": 15,
        "long_ma_period": 200,
        "require_long_ma": False,
        "pretrend_bars": 80,
        "min_prior_rise_pct": 25.0,
        "zigzag_threshold_pct": 3.5,
        "required_contractions": 3,
        "min_t1_pct": 8.0,
        "min_t2_pct": 4.5,
        "max_final_contraction_pct": 11.0,
        "contraction_ratio_max": 0.88,
        "min_final_low_age": 2,
        "max_final_low_age": 20,
        "tight_bars": 4,
        "tight_range_max_pct": 7.0,
        "volume_dryup_ratio_max": 0.90,
        "pivot_near_base_high_pct": 12.0,
        "monitor_distance_pct": 4.0,
        "breakout_margin_pct": 0.0,
        "breakout_volume_min": 1.3,
        "close_position_min": 0.50,
        "rs_lookback": 60,
        "rs_min_excess_pct": 0.0,
    ...
    "ゆるめ": {
        ...
        "required_contractions": 2,
        "min_t1_pct": 6.0,
        "min_t2_pct": 3.5,
        "max_final_contraction_pct": 14.0,
        "contraction_ratio_max": 0.96,
        "min_final_low_age": 1,
        "max_final_low_age": 25,
        "tight_bars": 3,
        "tight_range_max_pct": 9.0,
        "volume_dryup_ratio_max": 1.05,
        "pivot_near_base_high_pct": 16.0,
        "monitor_distance_pct": 5.0,
        "breakout_margin_pct": 0.0,
        "breakout_volume_min": 1.0,
        "close_position_min": 0.45,
        "rs_lookback": 60,
        "rs_min_excess_pct": -3.0,
...
VCP_WEEKLY_PRESETS = {
    "厳しめ": {
        ...
        "tight_range_max_pct": 7.0,
        "volume_dryup_ratio_max": 0.75,
        "pivot_near_base_high_pct": 10.0,
        "monitor_distance_pct": 3.5,
        "breakout_margin_pct": 0.15,
        "breakout_volume_min": 1.5,
        "close_position_min": 0.60,
        "rs_lookback": 12,
        "rs_min_excess_pct": 4.0,
        "max_stop_loss_pct": 10.0,
    },
    "普通": {
        "base_scan_bars": 36,
        ...
        "pretrend_bars": 35,
        "min_prior_rise_pct": 25.0,
        "zigzag_threshold_pct": 6.0,
        "required_contractions": 3,
        "min_t1_pct": 10.0,
        "min_t2_pct": 6.0,
        "max_final_contraction_pct": 13.0,
        "contraction_ratio_max": 0.88,
        "min_final_low_age": 1,
        "max_final_low_age": 10,
        "tight_bars": 3,
        "tight_range_max_pct": 9.0,
        "volume_dryup_ratio_max": 0.95,
        "pivot_near_base_high_pct": 14.0,
        "monitor_distance_pct": 5.0,
        "breakout_margin_pct": 0.0,
        "breakout_volume_min": 1.2,
        "close_position_min": 0.50,
    ...
    "ゆるめ": {
        ...
        "min_t1_pct": 8.0,
        "min_t2_pct": 4.0,
        "max_final_contraction_pct": 16.0,
        "contraction_ratio_max": 0.96,
        "min_final_low_age": 1,
        "max_final_low_age": 12,
        "tight_bars": 2,
        "tight_range_max_pct": 11.0,
        "volume_dryup_ratio_max": 1.08,
        "pivot_near_base_high_pct": 18.0,
        "monitor_distance_pct": 6.0,
        "breakout_margin_pct": 0.0,
        "breakout_volume_min": 1.0,
        "close_position_min": 0.45,
        "rs_lookback": 12,
...
@st.cache_data(ttl=60 * 60 * 6, show_spinner=False)
def download_market_benchmark(period):
    """
    相対強度比較のためにTOPIXを優先取得。
    データ取得が不安定なときは日経225を代替ベンチマークにする。
    """
    for ticker, label in [("^TPX", "TOPIX"), ("^N225", "日経225（代替）")]:
        try:
            df = yf.download(
                ticker,
                period=period,
            ...
            df = df[required_cols].copy().dropna(subset=["Close"])
            try:
                if df.index.tz is not None:
                    df.index = df.index.tz_localize(None)
            except Exception:
    ...
    return pd.DataFrame(), "指数データ取得不可"


def calc_relative_strength_vs_index(stock_df, benchmark_df, lookback):
    """
    Minerviniの独自RSレーティングではなく、
    指定期間の株価リターンからTOPIX（取得不可時は日経225）を引いた相対強度差を出す。
    """
    if (
        stock_df is None or stock_df.empty
        or benchmark_df is None or benchmark_df.empty
        or len(stock_df) < lookback + 1
    ):
        return np.nan, np.nan, np.nan

    stock_close = stock_df["Close"].astype(float).copy()
    benchmark_close = benchmark_df["Close"].astype(float).copy()

    benchmark_on_stock_dates = benchmark_close.reindex(stock_close.index, method="ffill")
    joined = pd.DataFrame({
        "stock": stock_close,
        "benchmark": benchmark_on_stock_dates,
    }).dropna()

    if len(joined) < lookback + 1:
        return np.nan, np.nan, np.nan

    start = joined.iloc[-lookback - 1]
    end = joined.iloc[-1]

    if start["stock"] <= 0 or start["benchmark"] <= 0:
        return np.nan, np.nan, np.nan

    stock_return_pct = (end["stock"] / start["stock"] - 1) * 100
    benchmark_return_pct = (end["benchmark"] / start["benchmark"] - 1) * 100
    rs_excess_pct = stock_return_pct - benchmark_return_pct

    return float(rs_excess_pct), float(stock_return_pct), float(benchmark_return_pct)


def find_vcp_zigzag_turns(df, threshold_pct):
    """
    高値から一定%以上下落したらピーク、安値から一定%以上上昇したらボトムとして記録。
    VCPのT1/T2/T3を機械的に近似するための簡易ZigZag。
    """
    ...
    turns = []
    state = 0  # 0=初期, 1=上昇局面, -1=下落局面

    peak_val = float(high[0])
    peak_idx = 0
    trough_val = float(low[0])
    trough_idx = 0
    ...
    for i in range(1, len(df)):
        if state >= 0:
            if float(high[i]) >= peak_val:
                peak_val = float(high[i])
                peak_idx = i

            if float(low[i]) <= peak_val * (1 - threshold):
                turns.append(("H", peak_idx, peak_val))
                state = -1
                trough_val = float(low[i])
                trough_idx = i
                continue

        if state <= 0:
            if float(low[i]) <= trough_val:
                trough_val = float(low[i])
                trough_idx = i

            if float(high[i]) >= trough_val * (1 + threshold):
                turns.append(("L", trough_idx, trough_val))
                state = 1
                peak_val = float(high[i])
                peak_idx = i
                continue
    ...
    # 同一種別が連続した場合は、より極端な方だけ残す
    cleaned = []
    for turn_type, idx, value in turns:
        if not cleaned:
            cleaned.append((turn_type, idx, value))
            continue

        prev_type, prev_idx, prev_value = cleaned[-1]
        if turn_type != prev_type:
            cleaned.append((turn_type, idx, value))
        elif turn_type == "H" and value >= prev_value:
            cleaned[-1] = (turn_type, idx, value)
        elif turn_type == "L" and value <= prev_value:
            cleaned[-1] = (turn_type, idx, value)

    return cleaned


def extract_vcp_contraction_legs(df, params):
    """
    最後の2〜3回のピーク→ボトムをT1/T2/T3候補として取り出す。
    """
    base_df = df.tail(params["base_scan_bars"]).copy().reset_index(drop=True)
    offset = len(df) - len(base_df)

    turns = find_vcp_zigzag_turns(base_df, params["zigzag_threshold_pct"])
    legs = []

    for a, b in zip(turns[:-1], turns[1:]):
        if a[0] == "H" and b[0] == "L" and b[1] > a[1]:
            depth_pct = (a[2] - b[2]) / a[2] * 100 if a[2] > 0 else np.nan
            if pd.notna(depth_pct) and depth_pct > 0:
                legs.append({
                    "peak_idx": int(a[1] + offset),
                    "peak_price": float(a[2]),
                    "trough_idx": int(b[1] + offset),
                    "trough_price": float(b[2]),
                    "depth_pct": float(depth_pct),
                })

    required = params["required_contractions"]
    if len(legs) < required:
        return []

    return legs[-required:]


def detect_recent_pocket_pivot(df, lookback=10):
    """
    直近で、上昇日に出来高が過去の下落日出来高を上回った日があるかを簡易判定。
    公式定義を完全再現するものではなく、VCPの需要確認用の補助指標。
    """
    ...
    start_i = max(lookback, 1)
    for i in range(start_i, len(df)):
        previous_window_start = max(1, i - lookback)
        down_day_volumes = [
            float(volume.iloc[j])
            for j in range(previous_window_start, i)
        ...
        if (
            ...
            and float(volume.iloc[i]) > max(down_day_volumes)
        ):
            return True, str(df.index[i].date())

    return False, ""


def calc_vcp_tightness(df, final_trough_idx, params):
    """
    T3後のタイトゾーンと出来高枯渇を判定。
    ブレイク当日は除外して、直前のコイリングを評価する。
    """
    n = len(df)
    tight_bars = params["tight_bars"]

    if final_trough_idx >= n - 1:
        return None

    # 監視候補は最新足まで、ブレイク候補は直前足までのタイトさを共通評価する。
    tight_end = n - 1
    tight_start = max(final_trough_idx + 1, tight_end - tight_bars)
    tight_df = df.iloc[tight_start:tight_end].copy()

    if len(tight_df) < max(2, tight_bars - 1):
        return None

    pivot_proxy = float(tight_df["High"].max())
    if pivot_proxy <= 0:
        return None

    tight_range_pct = (
        (float(tight_df["High"].max()) - float(tight_df["Low"].min()))
        / pivot_proxy * 100
    )

    compare_bars = max(tight_bars * 3, 10)
    compare_start = max(0, tight_start - compare_bars)
    compare_df = df.iloc[compare_start:tight_start].copy()

    tight_volume = float(tight_df["Volume"].astype(float).mean())
    prior_volume = float(compare_df["Volume"].astype(float).mean()) if not compare_df.empty else np.nan
    volume_dryup_ratio = tight_volume / prior_volume if pd.notna(prior_volume) and prior_volume > 0 else np.nan

    is_tight = tight_range_pct <= params["tight_range_max_pct"]
    is_dryup = pd.isna(volume_dryup_ratio) or volume_dryup_ratio <= params["volume_dryup_ratio_max"]

    return {
        "tight_range_pct": float(tight_range_pct),
        "volume_dryup_ratio": float(volume_dryup_ratio) if pd.notna(volume_dryup_ratio) else np.nan,
        "is_tight": bool(is_tight),
        "is_dryup": bool(is_dryup),
        "tight_start_idx": int(tight_start),
        "tight_end_idx": int(tight_end - 1),
    }
...
def analyze_vcp(
    ...
    """
    VCPを以下の順で評価する。
    1. Stage 2（EMAの上、EMA上向き）
    2. ベース前の上昇トレンド
    3. 2〜3回の収縮が小さくなっている
    4. T3後のタイトゾーンと出来高枯渇
    5. ピボット直前または出来高を伴うブレイク
    """
    if price_df is None or price_df.empty:
        return None

    min_required = max(
        params["base_scan_bars"] + 5,
        params["ema_period"] + params["ema_slope_bars"] + 2,
        params["long_ma_period"] + 2,
        params["pretrend_bars"] + 10,
    )
    ...
    close = df["Close"].astype(float)
    df["TrendEMA"] = close.ewm(span=params["ema_period"], adjust=False).mean()
    df["LongMA"] = close.rolling(params["long_ma_period"]).mean()
    ...
    trend_ema = float(df["TrendEMA"].iloc[-1])
    trend_ema_prev = float(df["TrendEMA"].iloc[-1 - params["ema_slope_bars"]])
    long_ma = float(df["LongMA"].iloc[-1]) if pd.notna(df["LongMA"].iloc[-1]) else np.nan

    above_trend_ema = latest_close > trend_ema
    trend_ema_up = trend_ema > trend_ema_prev
    above_long_ma = pd.isna(long_ma) or latest_close > long_ma

    stage2_ok = above_trend_ema and trend_ema_up
    if params["require_long_ma"]:
        stage2_ok = stage2_ok and above_long_ma

    if use_ma200_filter and timeframe_label == "日足":
        ma200 = close.rolling(200).mean().iloc[-1]
        if pd.isna(ma200) or latest_close < float(ma200):
            return None

    legs = extract_vcp_contraction_legs(df, params)
    if not legs:
        return None

    contractions = [x["depth_pct"] for x in legs]
    final_leg = legs[-1]
    final_trough_idx = final_leg["trough_idx"]
    final_trough_price = final_leg["trough_price"]
    final_low_age = len(df) - 1 - final_trough_idx

    # T1/T2/T3が順に浅くなるかを判定
    contraction_ok = True
    for earlier, later in zip(contractions[:-1], contractions[1:]):
        if later > earlier * params["contraction_ratio_max"]:
            contraction_ok = False
            break

    if len(contractions) >= 2:
        contraction_ok = contraction_ok and contractions[0] >= params["min_t1_pct"] and contractions[1] >= params["min_t2_pct"]

    final_contraction_ok = contractions[-1] <= params["max_final_contraction_pct"]
    final_low_age_ok = (
        params["min_final_low_age"] <= final_low_age <= params["max_final_low_age"]
    )

    # ベース前の上昇率：T1のピーク前の安値から最初の収縮ピークまで。
    first_peak_idx = legs[0]["peak_idx"]
    pretrend_start = max(0, first_peak_idx - params["pretrend_bars"])
    pretrend_low = float(df["Low"].iloc[pretrend_start:first_peak_idx + 1].min())
    first_peak_price = float(legs[0]["peak_price"])
    prior_rise_pct = (
        (first_peak_price / pretrend_low - 1) * 100
        if pretrend_low > 0 else np.nan
    )
    prior_rise_ok = pd.notna(prior_rise_pct) and prior_rise_pct >= params["min_prior_rise_pct"]

    # T3安値以降の高値をピボット候補として扱う。今日の高値は除外する。
    pivot_start = min(final_trough_idx + 1, len(df) - 1)
    pivot_df = df.iloc[pivot_start:-1].copy()

    if pivot_df.empty:
        return None

    pivot_price = float(pivot_df["High"].max())
    pivot_idx = pivot_df["High"].idxmax()
    base_high = max(x["peak_price"] for x in legs)
    pivot_near_base_ok = pivot_price >= base_high * (1 - params["pivot_near_base_high_pct"] / 100)

    tightness = calc_vcp_tightness(df, final_trough_idx, params)
    if tightness is None:
        return None

    rs_excess_pct, stock_return_pct, benchmark_return_pct = calc_relative_strength_vs_index(
        df,
        benchmark_df,
        params["rs_lookback"]
    )
    rs_ok = (
        pd.isna(rs_excess_pct)
        or rs_excess_pct >= params["rs_min_excess_pct"]
    )
    if not require_rs:
        rs_ok = True

    previous_volume_avg = float(df["Volume"].iloc[-11:-1].mean()) if len(df) >= 11 else np.nan
    latest_volume_ratio = (
        latest_volume / previous_volume_avg
        if pd.notna(previous_volume_avg) and previous_volume_avg > 0 else np.nan
    )

    breakout_line = pivot_price * (1 + params["breakout_margin_pct"] / 100)
    is_breakout = (
        latest_close > breakout_line
        and latest_close > latest_open
        and close_position >= params["close_position_min"]
        and (
            pd.isna(latest_volume_ratio)
            or latest_volume_ratio >= params["breakout_volume_min"]
        )
    )

    distance_to_pivot_pct = (
        (latest_close / pivot_price - 1) * 100
        if pivot_price > 0 else np.nan
    )
    is_near_pivot = (
        pd.notna(distance_to_pivot_pct)
        and -params["monitor_distance_pct"] <= distance_to_pivot_pct <= params["breakout_margin_pct"]
    )

    pocket_pivot, pocket_pivot_date = detect_recent_pocket_pivot(
        df.tail(max(params["tight_bars"] + 12, 15)),
        lookback=10
    )
    ...
    core_ok = (
        ...
        and prior_rise_ok
        and contraction_ok
        and final_contraction_ok
        and final_low_age_ok
        and pivot_near_base_ok
        and tightness["is_tight"]
        and tightness["is_dryup"]
        and rs_ok
    )
    ...
    stop_loss = final_trough_price * 0.99
    risk = entry_price - stop_loss
    risk_pct = risk / entry_price * 100 if entry_price > 0 else np.nan

    if pd.isna(risk_pct) or risk_pct <= 0 or risk_pct > params["max_stop_loss_pct"]:
        return None

    target_1 = entry_price + risk * 2
    target_2 = entry_price + risk * 3
    rr1 = (target_1 - entry_price) / risk if risk > 0 else np.nan
    rr2 = (target_2 - entry_price) / risk if risk > 0 else np.nan

    if is_breakout:
        setup_type = "VCPブレイク"
        buy_judge = "候補"
        entry_method = "ピボットを出来高を伴って終値で上抜け"
        comment = "T3のタイトゾーンを上抜け。翌日以降はピボットを割らないか確認。"
    elif is_near_pivot:
        setup_type = "VCP監視"
        buy_judge = "監視"
        entry_method = "ピボット上抜けでエントリー"
        comment = "T3でタイト化・出来高枯渇。ピボット上抜け待ち。"
    else:
        return None

    score = 0.0
    score += min(max(prior_rise_pct, 0), 60) / 60 * 20
    score += max(0, 1 - (contractions[-1] / max(contractions[0], 0.1))) * 20
    score += max(0, 1 - tightness["tight_range_pct"] / max(params["tight_range_max_pct"], 0.1)) * 15
    if pd.notna(tightness["volume_dryup_ratio"]):
        score += max(0, 1 - tightness["volume_dryup_ratio"]) * 15
    if pd.notna(rs_excess_pct):
        score += min(max(rs_excess_pct, 0), 20) / 20 * 15
    if pocket_pivot:
        score += 5
    if is_breakout:
        score += 12
    ...
    t1 = contractions[0] if len(contractions) >= 1 else np.nan
    t2 = contractions[1] if len(contractions) >= 2 else np.nan
    t3 = contractions[2] if len(contractions) >= 3 else np.nan
    ...
    return {
        ...
        "優先度": priority,
        "エントリー方法": entry_method,
        "上抜けライン": round(entry_price, 1),
        "T3安値": round(final_trough_price, 1),
        "損切りライン": round(stop_loss, 1),
        "第1利確ライン": round(target_1, 1),
        "第2利確ライン": round(target_2, 1),
        "想定損失%": round(risk_pct, 2),
        "第1RR": round(rr1, 2) if pd.notna(rr1) else np.nan,
        "第2RR": round(rr2, 2) if pd.notna(rr2) else np.nan,
        "コメント": comment,
        "最新日": latest_date.strftime("%Y-%m-%d"),
        "終値": round(latest_close, 1),
        "基準EMA": round(trend_ema, 1),
        "長期MA": round(long_ma, 1) if pd.notna(long_ma) else np.nan,
        "Stage2": "はい" if stage2_ok else "いいえ",
        "ベース前上昇率%": round(prior_rise_pct, 2) if pd.notna(prior_rise_pct) else np.nan,
        "収縮回数": len(contractions),
        "T1調整率%": round(t1, 2) if pd.notna(t1) else np.nan,
        "T2調整率%": round(t2, 2) if pd.notna(t2) else np.nan,
        "T3調整率%": round(t3, 2) if pd.notna(t3) else np.nan,
        "最終収縮率%": round(contractions[-1], 2),
        "タイトレンジ%": round(tightness["tight_range_pct"], 2),
        "出来高枯渇率": round(tightness["volume_dryup_ratio"], 2) if pd.notna(tightness["volume_dryup_ratio"]) else np.nan,
        "ブレイク出来高倍率": round(latest_volume_ratio, 2) if pd.notna(latest_volume_ratio) else np.nan,
        "Pocket Pivot": "あり" if pocket_pivot else "なし",
        "Pocket Pivot日": pocket_pivot_date,
        f"{benchmark_label}相対強度差%": round(rs_excess_pct, 2) if pd.notna(rs_excess_pct) else np.nan,
        "株価リターン%": round(stock_return_pct, 2) if pd.notna(stock_return_pct) else np.nan,
        f"{benchmark_label}リターン%": round(benchmark_return_pct, 2) if pd.notna(benchmark_return_pct) else np.nan,
        "ピボットまで%": round(distance_to_pivot_pct, 2) if pd.notna(distance_to_pivot_pct) else np.nan,
        "最終収縮安値からの日数": int(final_low_age),
        "スコア": round(score, 2),
        "Yahooチャート": f"https://finance.yahoo.co.jp/quote/{code}.T/chart",
    }


def show_saved_vcp_results():
    result_key = "screen_result_df_vcp"
    failed_key = "screen_failed_count_vcp"
    market_key = "screen_market_vcp"
    ...
    if result_df is None or result_df.empty:
        st.warning("VCP条件に一致する銘柄は見つかりませんでした。最初は「普通」または「ゆるめ」、確認銘柄数100〜300で試してください。")
        return
    ...
    with col1:
        judge_filter = st.selectbox(
            "表示する候補",
            ["すべて", "候補のみ", "監視のみ", "候補・監視"],
            key="judge_filter_vcp"
        )
    with col2:
        setup_filter = st.multiselect(
            "表示するセットアップ",
            ["VCPブレイク", "VCP監視"],
            default=["VCPブレイク", "VCP監視"],
            key="setup_filter_vcp"
        )
    with col3:
        timeframe_options = [
            x for x in ["日足", "週足"]
            if x in result_df.get("足種", pd.Series(dtype=str)).astype(str).unique().tolist()
        ]
        timeframe_filter = st.multiselect(
            "表示する足種",
            timeframe_options,
            default=timeframe_options,
            key="timeframe_filter_vcp"
        )
    ...
    st.download_button(
        label="CSVダウンロード",
        data=csv,
        file_name=f"vcp_{market_for_filename}_trade_plan.csv",
        mime="text/csv",
        key=f"download_vcp_{market_for_filename}"
    )
...
def run_vcp_screener(
    ...
    daily_benchmark, daily_benchmark_label = (
        download_market_benchmark("2y") if scan_daily else (pd.DataFrame(), "TOPIX")
    )
    weekly_benchmark, weekly_benchmark_label = (
        download_market_benchmark("5y") if scan_weekly else (pd.DataFrame(), "TOPIX")
    )
    if scan_weekly and weekly_benchmark is not None and not weekly_benchmark.empty:
        weekly_benchmark = resample_to_weekly_ohlcv(weekly_benchmark)
    ...
    if require_rs:
        labels = []
        if scan_daily:
            labels.append(f"日足：{daily_benchmark_label}")
        if scan_weekly:
            labels.append(f"週足：{weekly_benchmark_label}")
        st.caption("相対強度の比較指数：" + " / ".join(labels))
    ...
    for n, (_, row) in enumerate(target_stocks.iterrows(), start=1):
        ...
        try:
            if scan_daily:
                daily_params = VCP_DAILY_PRESETS[preset_name]
                daily_df = download_price(code)
                result = analyze_vcp(
                    price_df=daily_df,
                    benchmark_df=daily_benchmark,
                    benchmark_label=daily_benchmark_label,
                    code=code,
                    name=name,
                    market=market,
                    params=daily_params,
            ...
            if scan_weekly:
                weekly_params = VCP_WEEKLY_PRESETS[preset_name]
                long_daily_df = download_price_long(code)
                weekly_df = resample_to_weekly_ohlcv(long_daily_df)
                result = analyze_vcp(
                    price_df=weekly_df,
                    benchmark_df=weekly_benchmark,
                    benchmark_label=weekly_benchmark_label,
                    code=code,
                    name=name,
                    market=market,
                    params=weekly_params,
    ...
    status_area.write("VCPスクリーニング完了")
    ...
    st.session_state[failed_key] = failed_count
    st.session_state[market_key] = market_choice
    show_saved_vcp_results()


def render_vcp_tab(key_prefix):
    st.subheader("VCP（ボラティリティ・コントラクション・パターン）")

    st.write(
        "上昇トレンド後に、T1 → T2 → T3と調整幅と出来高が小さくなり、"
        "最後のタイトゾーンをピボット上抜けする候補を探します。"
    )

    st.info(
        "VCPは見た目だけでなく、売り圧力が段階的に枯れていることが重要です。"
        "このタブでは、ベース前の上昇・Stage 2・収縮の浅まり・T3タイト化・"
        "出来高枯渇・相対強度・Pocket Pivotをまとめて確認します。"
    )

    st.caption(
        "注意：Pocket PivotとVCPの波形は裁量判定の部分があります。"
        "アプリは候補を絞る目的であり、抽出後はチャートでT1/T2/T3とピボットを必ず確認してください。"
    )
    ...
    with col3:
        require_rs = st.checkbox(
            "TOPIXより強い銘柄に絞る",
            value=True,
            key=f"{key_prefix}_require_rs"
        )

    params = (
        VCP_WEEKLY_PRESETS[preset_name]
        if timeframe_choice == "週足"
        else VCP_DAILY_PRESETS[preset_name]
    )
    unit = "週" if timeframe_choice == "週足" else "営業日"

    st.write("現在の主な条件")
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("ベース前上昇", f"{params['min_prior_rise_pct']:.0f}%以上")
    c2.metric("収縮", f"{params['required_contractions']}回")
    c3.metric("T3最大調整", f"{params['max_final_contraction_pct']:.0f}%")
    c4.metric("タイトゾーン", f"{params['tight_bars']}{unit} / {params['tight_range_max_pct']:.0f}%以内")

    st.caption(
        f"Stage 2：価格が{params['ema_period']}{'週' if timeframe_choice == '週足' else '日'}EMAより上、"
        f"EMA上向き。ピボットから{params['monitor_distance_pct']:.1f}%以内を監視し、"
        f"出来高{params['breakout_volume_min']:.1f}倍以上を伴う上抜けを候補にします。"
    )

    if timeframe_choice == "日足・週足の両方":
        st.caption("日足・週足は別条件で判定します。結果の「足種」列で分けて確認できます。")
...
# ============================================================
# MACD ゴールデンクロス直前スクリーニング
# MACD：短期EMA 12、長期EMA 26、シグナル 9
# ============================================================
# 「GC直前」は、MACDがまだシグナルを下回っている一方で、
# 差（ヒストグラム）がゼロへ近づき、MACD・ヒストグラムが連続上昇している状態。
# 単純なMACD接近だけではダマシが多いため、価格位置・出来高・損切り幅も併用する。
MACD_DAILY_PRESETS = {
    "厳しめ": {
        "trend_ema_period": 20,
        "long_ma_period": 50,
        "require_above_trend_ema": True,
    ...
    "ゆるめ": {
        "trend_ema_period": 20,
        "long_ma_period": 50,
        "require_above_trend_ema": False,
        "require_above_long_ma": False,
        "rise_bars": 2,
        "max_gap_atr": 0.35,
        "volume_ratio_min": 0.40,
        "close_position_min": 0.40,
        "entry_lookback": 5,
        "stop_lookback": 8,
        "max_stop_loss_pct": 13.0,
    },
...
MACD_WEEKLY_PRESETS = {
    ...
    "ゆるめ": {
        ...
        "entry_lookback": 4,
        "stop_lookback": 5,
        "max_stop_loss_pct": 18.0,
    },
}


def calculate_macd_12_26_9(close):
    """MACD(12, 26, 9)を計算する。"""
    ...
    return ema12, ema26, macd_line, signal_line, histogram


def calculate_atr(df, period=20):
    """価格水準が異なる銘柄でもMACD差を比較できるようATRを計算する。"""
    high = df["High"].astype(float)
    low = df["Low"].astype(float)
...
def is_strictly_rising(series, bars):
    """直近bars本が連続で上昇しているか。"""
    if series is None or len(series) < bars:
        return False

    values = series.iloc[-bars:].astype(float).to_numpy()
...
def analyze_macd_gc(
    ...
    params,
    timeframe_label,
    include_gc_today=True,
    use_ma200_filter=False
):
    """
    MACD(12,26,9)のGC直前を抽出する。

    GC直前：
    ・MACD < シグナル（まだGC前）
    ・ヒストグラムがマイナス圏で連続上昇
    ・MACD本体も連続上昇
    ・MACDとシグナルの差がATR比で小さい
    ...
    min_length = max(
        60,
        26 + 9 + params["rise_bars"] + 5,
        params["long_ma_period"] + 5,
        params["stop_lookback"] + 5
    )
    if len(price_df) < min_length:
        return None
    ...
    close = df["Close"]
    df["EMA12"], df["EMA26"], df["MACD"], df["Signal"], df["Histogram"] = calculate_macd_12_26_9(close)
    df["ATR20"] = calculate_atr(df, period=20)
    df["TrendEMA"] = close.ewm(span=params["trend_ema_period"], adjust=False).mean()
    df["LongMA"] = close.rolling(params["long_ma_period"]).mean()

    latest = df.iloc[-1]
    previous = df.iloc[-2]

    if pd.isna(latest["ATR20"]) or float(latest["ATR20"]) <= 0:
        return None
    ...
    hist_now = float(latest["Histogram"])
    macd_prev = float(previous["MACD"])
    signal_prev = float(previous["Signal"])
    hist_prev = float(previous["Histogram"])
    atr_now = float(latest["ATR20"])

    trend_ema = float(latest["TrendEMA"])
    long_ma = float(latest["LongMA"]) if pd.notna(latest["LongMA"]) else np.nan
    close_position = calc_close_position(latest)

    price_above_trend_ema = latest_close >= trend_ema
    price_above_long_ma = pd.isna(long_ma) or latest_close >= long_ma

    if params["require_above_trend_ema"] and not price_above_trend_ema:
        return None

    if params["require_above_long_ma"] and not price_above_long_ma:
        return None
    ...
    # MACDとSignalの距離をATRで正規化。
    # 0に近いほどGC直前とみなす。
    gap_atr = abs(macd_now - signal_now) / atr_now

    rise_bars = params["rise_bars"]
    macd_rising = is_strictly_rising(df["MACD"], rise_bars)
    histogram_rising = is_strictly_rising(df["Histogram"], rise_bars)

    # GC直前：まだ下にいるが、ヒストグラムはゼロに向かって改善。
    pre_gc = (
        macd_now < signal_now
        and hist_now < 0
        and gap_atr <= params["max_gap_atr"]
        and macd_rising
        and histogram_rising
    )

    # GC当日：確認用。デフォルトでは結果に含めるが、UIで直前だけに絞れる。
    gc_today = (
    ...
    if not pre_gc and not (include_gc_today and gc_today):
        return None

    # 直近の出来高・ローソク足位置も弱すぎるものを除外。
    average_volume_5 = float(df["Volume"].iloc[-6:-1].mean()) if len(df) >= 6 else np.nan
    volume_ratio = (
        float(latest["Volume"]) / average_volume_5
        if pd.notna(average_volume_5) and average_volume_5 > 0 else np.nan
    )
    ...
    if latest_close < latest_open and close_position < params["close_position_min"]:
        return None

    # エントリーはGC確認後、直近の小さな戻り高値を抜く形を想定。
    entry_lookback = params["entry_lookback"]
    entry_window = df.iloc[-entry_lookback:].copy()
    entry_line = float(entry_window["High"].max())

    stop_lookback = params["stop_lookback"]
    stop_line = float(df["Low"].iloc[-stop_lookback:].min()) * 0.99

    risk = entry_line - stop_line
    risk_pct = risk / entry_line * 100 if entry_line > 0 else np.nan

    if pd.isna(risk_pct) or risk_pct <= 0 or risk_pct > params["max_stop_loss_pct"]:
        return None

    target_1 = entry_line + risk * 2
    target_2 = entry_line + risk * 3

    if pre_gc:
        setup_type = "GC直前"
        buy_judge = "監視"
        entry_method = f"MACDのGC確認後、直近{entry_lookback}{'週' if timeframe_label == '週足' else '日'}高値を上抜け"
        comment = "MACDはまだシグナルの下だが、差が縮小。MACD・ヒストグラムが上向きでGC接近。"
    else:
        setup_type = "GC当日"
        buy_judge = "候補"
        entry_method = f"MACDがGC。直近{entry_lookback}{'週' if timeframe_label == '週足' else '日'}高値を上抜けているか確認"
        comment = "MACDがシグナルを上抜け。出来高・終値位置・上値抵抗をチャートで確認。"

    # 0に近いほど高い「GC接近度」。
    closeness = max(0.0, 1.0 - gap_atr / params["max_gap_atr"]) if params["max_gap_atr"] > 0 else 0.0

    # 優先度スコア。GC直前は近さ、GC当日は確認の強さを重視。
    score = 0.0
    score += closeness * 45
    score += min(max((hist_now - hist_prev) / atr_now, 0), 0.5) * 30
    score += 10 if price_above_trend_ema else 0
    score += 8 if price_above_long_ma else 0
    score += min(max(volume_ratio, 0), 2.0) * 4 if pd.notna(volume_ratio) else 0
    score += 10 if gc_today else 0
    score += 5 if close_position >= 0.65 else 0

    if gc_today and score >= 70:
        priority = "S"
    elif score >= 52:
        priority = "A"
    else:
    ...
    zero_line_position = "上" if macd_now >= 0 else "下"
    trend_text = (
        f"{params['trend_ema_period']}{'週' if timeframe_label == '週足' else '日'}EMAより上"
        if price_above_trend_ema else
        f"{params['trend_ema_period']}{'週' if timeframe_label == '週足' else '日'}EMAより下"
    )
    ...
    return {
        ...
        "シグナル": round(signal_now, 4),
        "ヒストグラム": round(hist_now, 4),
        "MACD差/ATR": round(gap_atr, 3),
        "GC接近度": round(closeness * 100, 1),
        "MACD連続上昇": f"{rise_bars}本" if macd_rising else "なし",
        "ヒストグラム連続改善": f"{rise_bars}本" if histogram_rising else "なし",
        "MACDゼロライン": zero_line_position,
        "トレンド位置": trend_text,
        "長期MAより上": "はい" if price_above_long_ma else "いいえ",
        "出来高倍率": round(volume_ratio, 2) if pd.notna(volume_ratio) else np.nan,
        "終値位置": round(close_position, 2),
        "スコア": round(score, 2),
        "Yahooチャート": f"https://finance.yahoo.co.jp/quote/{code}.T/chart",
...
def show_saved_macd_results():
    result_key = "screen_result_df_macd_gc"
    failed_key = "screen_failed_count_macd_gc"
    market_key = "screen_market_macd_gc"
    ...
    if result_df is None or result_df.empty:
        st.warning("条件に一致する銘柄は見つかりませんでした。最初は「普通」または「ゆるめ」、確認銘柄数100〜300で試してください。")
        return
    ...
    with col3:
        priority_filter = st.multiselect(
            ...
            ["S", "A", "B"],
            default=["S", "A", "B"],
            key="priority_filter_macd_gc"
        )

    display_df = result_df.copy()

    if setup_filter == "GC直前のみ":
        display_df = display_df[display_df["セットアップ"] == "GC直前"]
    elif setup_filter == "GC当日のみ":
        display_df = display_df[display_df["セットアップ"] == "GC当日"]

    if timeframe_filter:
        display_df = display_df[display_df["足種"].isin(timeframe_filter)]
    if priority_filter:
        display_df = display_df[display_df["優先度"].isin(priority_filter)]
    ...
    st.download_button(
        label="CSVダウンロード",
        data=csv,
        file_name=f"macd_gc_{market_for_filename}_watchlist.csv",
        mime="text/csv",
        key=f"download_macd_gc_{market_for_filename}"
    )


def run_macd_gc_screener(
    market_choice,
    preset_name,
    timeframe_choice,
    max_scan,
    ...
    for n, (_, row) in enumerate(target_stocks.iterrows(), start=1):
        ...
        try:
            if scan_daily:
                daily_df = download_price(code)
                result = analyze_macd_gc(
                    price_df=daily_df,
                    code=code,
                    name=name,
                    market=market,
                    params=MACD_DAILY_PRESETS[preset_name],
                    timeframe_label="日足",
                    include_gc_today=include_gc_today,
                    use_ma200_filter=use_ma200_filter
                )
                if result is not None:
                    result["33業種"] = row.get("33業種", "")
            ...
            if scan_weekly:
                ...
                result = analyze_macd_gc(
                    ...
                    name=name,
                    market=market,
                    params=MACD_WEEKLY_PRESETS[preset_name],
                    timeframe_label="週足",
                    include_gc_today=include_gc_today,
                    use_ma200_filter=False
                )
    ...
    status_area.write("MACD GC直前スクリーニング完了")
    ...
    if not results:
        ...
        result_df = result_df.sort_values(
            by=["_買い候補順", "_優先度順", "_足種順", "GC接近度", "スコア"],
            ascending=[True, True, True, False, False]
        ).reset_index(drop=True)
    ...
    st.session_state[failed_key] = failed_count
    st.session_state[market_key] = market_choice
    show_saved_macd_results()


def render_macd_gc_tab(key_prefix):
    st.subheader("MACD ゴールデンクロス直前（日足・週足）")

    st.write(
        "MACD（短期EMA 12・長期EMA 26・シグナル 9）が、"
        "シグナルを下から上抜ける直前の銘柄を探します。"
    )

    st.info(
        "単にMACDとシグナルが近いだけでは抽出しません。"
        "MACD・ヒストグラムの連続改善、ATRに対する差の小ささ、"
        "価格位置、出来高、損切り幅を合わせて確認します。"
    )

    st.caption(
        "GC直前は監視候補です。実際のエントリーは、MACDのGCと直近高値上抜けを確認してからを基本にしています。"
    )
    ...
    with col3:
        include_gc_today = st.checkbox(
            "GC当日も確認用として含める",
            value=True,
            key=f"{key_prefix}_include_today"
        )

    params = (
        MACD_WEEKLY_PRESETS[preset_name]
        if timeframe_choice == "週足"
        else MACD_DAILY_PRESETS[preset_name]
    )
    unit = "週" if timeframe_choice == "週足" else "日"

    st.write("現在の主な条件")
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("MACD差の上限", f"ATRの{params['max_gap_atr']:.2f}倍")
    c2.metric("MACD・ヒストグラム改善", f"{params['rise_bars']}本連続")
    c3.metric("トレンドEMA", f"{params['trend_ema_period']}{unit}")
    c4.metric("損切り許容幅", f"{params['max_stop_loss_pct']:.0f}%以内")

    st.caption(
        "GC直前：MACD < シグナル、ヒストグラムはマイナス圏で改善中。"
        "GC当日：前足までMACD <= シグナルで、当足に上抜けたものです。"
    )
...
st.title("25日線・75日線・200日線・ウェッジ・VCP・MACD スクリーニングアプリ")

st.write("出来高急増後の25日線初押し、75日線押し目、200日線押し目、下降ウェッジのWB/WBPB、VCP、MACDのGC直前を探すスクリーニングアプリです。ウェッジ・VCP・MACDは日足・週足に対応しています。")
...
with st.sidebar:
    ...
    use_ma200_filter = st.checkbox("25日線・75日線・日足ウェッジ・日足VCP・日足MACD：200日線より上の銘柄に絞る", value=False)
...
with st.expander("このアプリの見方"):
    st.write(
        ...
        VCPは、ベース前の上昇後に2〜3回の調整が順に浅くなり、T3のタイトゾーンをピボット上抜けする候補を探します。
        VCPはTOPIXとの相対強度、Stage 2、出来高枯渇、Pocket Pivotも補助条件として表示します。
        MACD GC直前は、MACD（12・26・9）がまだシグナルの下にある一方で、ヒストグラムとMACDが連続改善し、GCに近づいている銘柄を探します。

        ウェッジ・VCP・MACDの日足は短期〜数週間のスイング候補、週足は数週間〜数か月の中期候補を探す用途です。
...
tab25, tab75, tab200, tab_wedge, tab_vcp, tab_macd = st.tabs(["25日線初押し", "75日線押し目", "200日線押し目", "ウェッジブレイク（日足・週足）", "VCP（日足・週足）", "MACD GC直前（日足・週足）"])
