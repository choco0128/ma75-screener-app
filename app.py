...
st.title("25日線・75日線・200日線・ウェッジ・VCP スクリーニングアプリ")

st.write("出来高急増後の25日線初押し、75日線押し目、200日線押し目、下降ウェッジのWB/WBPB、VCPを探すスクリーニングアプリです。ウェッジとVCPは日足・週足に対応しています。")
...
with st.sidebar:
    ...
    use_ma200_filter = st.checkbox("25日線・75日線・日足ウェッジ・日足VCP：200日線より上の銘柄に絞る", value=False)
...
with st.expander("このアプリの見方"):
    st.write(
        ...
        ウェッジとVCPの日足は短期〜数週間のスイング候補、週足は数週間〜数か月の中期候補を探す用途です。
...
tab25, tab75, tab200, tab_wedge, tab_vcp = st.tabs(["25日線初押し", "75日線押し目", "200日線押し目", "ウェッジブレイク（日足・週足）", "VCP（日足・週足）"])
