...
def build_kabutan_news_queries(code, name, days=3):
    """
    値上がり理由分析では、まず株探（かぶたん）系の見出しを優先的に探す。
    Google News RSSは無料で使えるが、取得結果はGoogle側の配信状況に依存する。
    """
    d = int(days)
    ...
    if name:
        queries.extend([
            f'site:kabutan.jp "{name}" 株探 急騰 上昇 材料 when:{d}d',
            f'site:kabutan.jp "{name}" 決算 上方修正 増配 受注 提携 when:{d}d',
            f'株探 "{name}" 急騰 材料 when:{d}d',
        ])
    if code:
        queries.append(f'site:kabutan.jp {code} 株探 材料 決算 上方修正 when:{d}d')
    return queries
...
def build_company_news_queries(code, name, days=3):
    ...
    if name:
        queries.extend([
            ...
            f'"{name}" 決算 上方修正 増配 受注 提携 when:{d}d',
            f'"{name}" ストップ高 値上がり 理由 when:{d}d',
        ])
    if code:
        queries.append(f'{code} {name} 決算 上方修正 増配 受注 提携 when:{d}d')
    return queries


def is_kabutan_article(article):
    title = str(article.get("title", ""))
    source = str(article.get("source", ""))
    link = str(article.get("link", ""))
    joined = f"{title} {source} {link}".lower()
    return (
        "kabutan" in joined
        or "株探" in joined
        or "みんかぶ" in joined and "株探" in title
    )


@st.cache_data(ttl=60 * 30, show_spinner=False)
def fetch_kabutan_stock_news_direct(code, max_items=5):
    """
    株探の銘柄ニュースページを直接見に行く無料取得。
    取得できない場合もあるため、Google News検索と併用する。
    """
    if BeautifulSoup is None:
        return []
    ...
    urls = [
        f"https://kabutan.jp/stock/news?code={quote(code)}",
        f"https://kabutan.jp/stock/kabuka?code={quote(code)}",
    ]
    articles = []
    ...
    for url in urls:
        try:
            ...
            if res.status_code != 200:
                continue
            # 株探はShift_JIS系で返ることがあるため、apparent_encodingも見る
            ...
            for a in soup.find_all("a", href=True):
                ...
                if not title or len(title) < 8:
                    continue
                # ニュース系リンクだけ拾う
                if not any(x in href for x in ["/news/", "/stock/news"]):
                    continue
                # ナビゲーションや広告っぽい短文を除外
                if title in ["株探", "ニュース", "決算速報", "材料", "市況"]:
                    continue
                full_link = urljoin("https://kabutan.jp", href)
                articles.append({
                    "title": title,
                    "source": "株探",
                    "published": "",
    ...
    return dedupe_news_articles(articles)[: int(max_items)]


def summarize_articles_reason(articles, prefix="ニュース"):
    if not articles:
        return "", "", ""
    ...
    if keywords:
        reason = f"{prefix}見出しで「{'、'.join(keywords[:5])}」を確認。関連材料：{title_text}"
    else:
        reason = f"{prefix}見出しを確認：{title_text}"
...
def summarize_company_news_reason(code, name, news_days=3, max_items=3):
    ...
    値上がり理由用のニュース要約。
    1. 株探を優先
    2. 株探で取れない/弱い場合はその他ニュースで補完
    """
    kabutan_articles = []

    # 直接取得
    kabutan_articles.extend(fetch_kabutan_stock_news_direct(code, max_items=max_items))

    # Google News経由で株探を検索
    for q in build_kabutan_news_queries(code, name, days=news_days):
        kabutan_articles.extend(fetch_google_news_rss(q, max_items=max_items))

    kabutan_articles = dedupe_news_articles(kabutan_articles)
    kabutan_articles = [a for a in kabutan_articles if is_kabutan_article(a) or str(a.get("source", "")) == "株探"]
    kabutan_articles = kabutan_articles[: int(max_items)]

    general_articles = []
    for q in build_company_news_queries(code, name, days=news_days):
        general_articles.extend(fetch_google_news_rss(q, max_items=max_items))
    general_articles = dedupe_news_articles(general_articles)
    # 株探記事は重複するので一般側からは外す
    general_articles = [a for a in general_articles if not is_kabutan_article(a)][: int(max_items)]

    kabutan_reason, kabutan_evidence, kabutan_link = summarize_articles_reason(kabutan_articles, prefix="株探")
    general_reason, general_evidence, general_link = summarize_articles_reason(general_articles, prefix="その他ニュース")

    if kabutan_reason and general_reason:
        reason = f"株探優先：{kabutan_reason} / 補足：{general_reason}"
        evidence = " / ".join([x for x in [kabutan_evidence, general_evidence] if x])
        first_link = kabutan_link or general_link
    elif kabutan_reason:
        reason = f"株探優先：{kabutan_reason}"
        evidence = kabutan_evidence
        first_link = kabutan_link
    elif general_reason:
        reason = general_reason
        evidence = general_evidence
        first_link = general_link
    else:
        reason = "株探・その他ニュースで明確な個別材料は取得できず"
        evidence = ""
        first_link = ""

    return reason, evidence, first_link, kabutan_reason, general_reason
...
def run_top_gainers_reason_screening(
    ...
    for idx, row in result_df.iterrows():
        ...
        if use_news and idx < news_n:
            ...
            news_reason, news_evidence, news_link, kabutan_reason, other_news_reason = summarize_company_news_reason(
                code=code,
                name=name,
                news_days=int(news_days),
                max_items=4,
        ...
        out["かぶたん理由"] = kabutan_reason
        out["その他ニュース理由"] = other_news_reason
...
def render_top_gainers_reason_result():
    ...
    display_cols = [
        ...
        "推定上昇理由", "TDnet開示理由", "かぶたん理由", "その他ニュース理由", "ニュース理由", "価格出来高理由",
    ...
    st.write("### かぶたん理由")
    st.write(row.get("かぶたん理由", ""))

    st.write("### その他ニュース理由")
    st.write(row.get("その他ニュース理由", ""))
...
def render_top_gainers_reason_tab(tab_key="top_gainers_reason"):
    ...
    st.write(
        ...
        "各銘柄について、価格・出来高・高値更新・TDnet開示・かぶたん優先ニュース・その他ニュースから上昇理由を推定します。"
    )
    ...
    with col2:
        ...
        use_news = st.checkbox("ニュースも理由に使う（かぶたん優先＋その他ニュース補完）", value=True, key=f"{tab_key}_use_news")
    with col3:
    ...
    st.info("ニュース理由は、かぶたんを優先し、取れない場合はその他ニュースで補完します。全市場・ニュース50件検索は時間がかかるので、最初は確認銘柄数100〜300で動作確認してください。")
...
st.title("25日線初押し・75日線押し目・三角保ち合い・モメンタム・材料判定 スクリーニングアプリ")

st.write("出来高急増後の25日線初押し・75日線押し目・三角保ち合いブレイク初動・モメンタム初動/強勢継続・場中/引け後の適時開示PDF材料判定に加えて、材料インパクトランキング、値上がり率Top50理由分析をタブで切り替えて探せます。")
...
with st.expander("このアプリの見方"):
    st.write(
        ...
        モメンタムは、短期で強く上がり、高値圏で出来高が増えている銘柄を探す用途です。
        適時開示PDF材料判定は、場中・引け後に出たPDF本文を読み、好材料・悪材料・重要度を無料のルール判定で分類する用途です。
        材料インパクトランキングは、PDF本文から材料そのものの強さをS/A/Bで順位付けし、翌日監視優先度を出す用途です。
        値上がり率Top50理由分析は、最新取引日の上昇率上位50銘柄について、価格・出来高・TDnet開示・ネットニュースから上昇理由を推定する用途です。
        テーマ欄は、JPX業種・銘柄名・一部コードから推定した補助表示です。
...
tab25, tab75, tab_triangle, tab_momentum, tab_tdnet_pdf, tab_material_impact, tab_top_gainers = st.tabs(["25日線初押し", "75日線押し目", "三角保ち合いブレイク", "モメンタム", "適時開示PDF材料判定", "材料インパクトランキング", "値上がり率Top50理由分析"])
...
with tab_momentum:
    render_momentum_tab("momentum")

with tab_tdnet_pdf:
    render_tdnet_pdf_tab("tdnet_pdf")

with tab_material_impact:
    render_material_impact_tab("material_impact")

with tab_top_gainers:
    render_top_gainers_reason_tab("top_gainers_reason")
