...
def load_jpx_list():
    ...
    sector33_col = find_column(df.columns, "33業種")
    sector17_col = find_column(df.columns, "17業種")
...
def render_tdnet_pdf_tab(tab_key="tdnet_pdf"):
    ...
    st.write("無料で使える範囲として、非公式TDnet一覧からPDFを取得し、PDF本文をルール判定します。OpenAI APIキーやJ-Quants有料アドオンは不要です。")
    ...
    if mode == "当日の適時開示を取得して判定":
        ...
        with c2:
            after_time = st.selectbox("この時刻以降", ["15:00", "15:30", "16:00", "17:00", "00:00"], index=1, key=f"{tab_key}_after")
        with c3:
            max_items = st.number_input("取得する最大件数", min_value=20, max_value=500, value=200, step=20, key=f"{tab_key}_max_items")
        with c4:
            max_pdf_count = st.number_input("PDF本文を読む件数", min_value=5, max_value=120, value=40, step=5, key=f"{tab_key}_max_pdf")

        c5, c6, c7 = st.columns(3)
        with c5:
            max_pages = st.number_input("1PDFあたり読むページ数", min_value=1, max_value=20, value=6, step=1, key=f"{tab_key}_pages")
        with c6:
...
st.title("25日線初押し・75日線押し目・三角保ち合い・モメンタム・適時開示 スクリーニングアプリ")

st.write("出来高急増後の25日線初押し・75日線押し目・三角保ち合いブレイク初動・モメンタム初動/強勢継続・適時開示PDF材料判定をタブで切り替えて探せます。モメンタムは候補が多く出すぎないよう、厳しめ設定を強化しています。")
...
with st.expander("このアプリの見方"):
    st.write(
        ...
        適時開示PDF材料判定は、引け後に出たPDF本文を読み、好材料・悪材料・重要度を無料のルール判定で分類する用途です。
...
tab25, tab75, tab_triangle, tab_momentum, tab_tdnet_pdf = st.tabs(["25日線初押し", "75日線押し目", "三角保ち合いブレイク", "モメンタム", "適時開示PDF材料判定"])
