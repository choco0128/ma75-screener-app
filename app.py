...
def render_theme_flow_tab(tab_key="theme_flow"):
    ...
    st.write("個別銘柄名は表示せず、どのテーマに資金が入っているかをランキング形式で表示します。流入理由はネットニュースの見出しから探します。")
...
st.write("出来高急増後の25日線初押し・75日線押し目・三角保ち合いブレイク初動・モメンタム初動/強勢継続・場中/引け後の適時開示PDF材料判定に加えて、材料×モメンタム、テーマ別資金流入ランキングをタブで切り替えて探せます。")
...
with st.expander("このアプリの見方"):
    st.write(
        ...
        材料×モメンタムは、好材料PDFと現在のモメンタム状態を合成し、翌日監視優先度を出す用途です。
        テーマ別資金流入ランキングは、モメンタムの強い銘柄をテーマ別に集計し、今どこに資金が来ているかを見る用途です。
...
tab25, tab75, tab_triangle, tab_momentum, tab_tdnet_pdf, tab_material_momentum, tab_theme_flow = st.tabs(["25日線初押し", "75日線押し目", "三角保ち合いブレイク", "モメンタム", "適時開示PDF材料判定", "材料×モメンタム", "テーマ資金流入"])
...
with tab_material_momentum:
    render_material_momentum_tab("material_momentum")

with tab_theme_flow:
    render_theme_flow_tab("theme_flow")
