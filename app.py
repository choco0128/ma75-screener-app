...
PRESETS = {
    25: {
        "厳しめ": {"near_lower": -1.5, "near_upper": 2.5, "slope_lookback": 10, "slope_min": 0.8, "along_days": 4, "min_near_days": 2, "volume_ratio_max": 1.6},
        "普通": {"near_lower": -2.5, "near_upper": 4.0, "slope_lookback": 10, "slope_min": 0.3, "along_days": 4, "min_near_days": 2, "volume_ratio_max": 2.0},
        "ゆるめ": {"near_lower": -4.0, "near_upper": 6.0, "slope_lookback": 10, "slope_min": 0.0, "along_days": 4, "min_near_days": 1, "volume_ratio_max": 2.8},
    },
    75: {
        "厳しめ": {"near_lower": -1.5, "near_upper": 3.0, "slope_lookback": 20, "slope_min": 1.0, "along_days": 5, "min_near_days": 3, "volume_ratio_max": 1.4},
        "普通": {"near_lower": -2.5, "near_upper": 5.0, "slope_lookback": 20, "slope_min": 0.5, "along_days": 5, "min_near_days": 2, "volume_ratio_max": 1.8},
        "ゆるめ": {"near_lower": -4.0, "near_upper": 8.0, "slope_lookback": 20, "slope_min": 0.0, "along_days": 5, "min_near_days": 1, "volume_ratio_max": 2.5},
    },
}


st.set_page_config(page_title="移動平均線押し目スクリーニング", layout="wide")
...
def load_jpx_list():
    ...
    result["コード"] = (
        result["コード"].astype(str).str.strip().str.replace(r"\.0$", "", regex=True).str.upper()
    )
...
def analyze_ma_pullback(price_df, code, name, market, ma_period, params, use_volume_filter=True, use_ma200_filter=False):
    ...
    near_count = recent_dist.between(params["near_lower"], params["near_upper"]).sum()
    ...
    score = slope_pct * 2 - abs(dist_ma_pct) * 1.2 + near_count * 1.5 - max(0, volume_ratio - 1) * 1.0
...
def run_screener(ma_period, market_choice, preset_name, max_scan, wait_sec, use_volume_filter, use_ma200_filter):
    ...
    target_stocks = all_stocks[all_stocks["市場"].str.contains(market_choice, na=False)].copy()
    ...
    if len(results) == 0:
        ...
        result_df = result_df.sort_values(by=sort_cols, ascending=[False, False, True]).reset_index(drop=True)
        ...
        st.download_button(
            label="CSVダウンロード",
            data=csv,
            file_name=f"ma{ma_period}_pullback_{market_choice}.csv",
            mime="text/csv",
            key=f"download_ma{ma_period}",
        )
...
def render_tab(ma_period, key_prefix):
    ...
    with col1:
        preset_name = st.selectbox("判定の厳しさ", ["普通", "厳しめ", "ゆるめ"], key=f"{key_prefix}_preset")
    with col2:
        params = PRESETS[ma_period][preset_name]
        st.write("現在の条件")
    ...
    run_button = st.button(f"{ma_period}日線スクリーニング実行", type="primary", key=f"{key_prefix}_run")
...
with st.sidebar:
    st.header("共通設定")
    market_choice = st.selectbox("市場を選択", ["プライム", "スタンダード", "グロース"])
    use_volume_filter = st.checkbox("出来高が暴れすぎていない銘柄に絞る", value=True)
    use_ma200_filter = st.checkbox("200日線より上の銘柄に絞る", value=False)

    st.markdown("### 実行設定")
    max_scan = st.number_input("確認する銘柄数。0なら選択市場すべて", min_value=0, max_value=4000, value=100, step=50)
    wait_sec = st.number_input("1銘柄ごとの待機秒数", min_value=0.0, max_value=2.0, value=0.1, step=0.1)
